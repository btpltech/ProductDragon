...
#hbase-site.xml conf
sudo rm -f $HBASE_HOME/conf/hbase-site.xml
echo "creating hbase-site.xml script"
...
ssh $SSH_OPTS "ubuntu@$HOSTNAME" sudo chmod -R 777 /etc
...
if [ "$IS_MASTER" == "true" ]; then
 ...
 sudo  mkdir -p /mnt/ganglia/rrds
  sudo chown -R ganglia:ganglia /mnt/ganglia/rrds
  sudo rm -rf /var/lib/ganglia; cd /var/lib; ln -s /mnt/ganglia ganglia; cd
  service gmond start
...
ssh $SSH_OPTS "ubuntu@$HOSTNAME" sudo chmod -R 751 /etc
# Run this script on next boot
sudo rm -f /var/ec2/ec2-run-user-data.*
