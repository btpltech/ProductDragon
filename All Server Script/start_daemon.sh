...
ec2-run-instances $AMI_IMAGE -n "1" -g "selenium" -k "$KEY_NAME" -t "$INSTANCE_TYPE" $KERNEL_ARG | grep INSTANCE | awk '{print $2}'>> $AMPLE_HOME/nutch/version2/selenium.txt
echo "Done"

FILE=$AMPLE_HOME/nutch/version2/selenium.txt
...
sudo rm -f $AMPLE_HOME/nutch/version2/selenium_instance_master.txt

if [ ! -z $SLAVE_HOST ]; then
   echo "Started"
  echo "$SLAVE_HOST"  >>$AMPLE_HOME/nutch/version2/selenium_master.txt
 break;
fi
sleep 1
done
